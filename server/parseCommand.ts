 ...
 * @returns 
 */
 export async function parseCommand(command: string, client: Client) {
    ...
    switch(args[0].toUpperCase()) {
      ...
      case "JOIN": {
        ...
        // Verify argument is there
        if(!args[1]) {
          await client.send("You must specify a room to join", ResponseType.ERROR );
          return;
        }

        // Add to the chat
        messenger.addClientToRoom(client, args[1]));
