    ...
    if(room) {
      ...
      for(let prompt of prompts) {
        if(prompt.pattern.test(input)) {
          ...
          setTimeout(() => {
            renderChat(0, "Welcome Bot", responses[Math.floor(Math.random() * responses.length)]);
          }, 200);
          return;
        }
      }
  ...
  switch(components[0]) {
    ...
    case "ID":
      if(!id && location.hash.length > 1) {
        ...
        renderChat(0, "Welcome Bot", "Welcome to carterjs chat! Type /help for a list of commands.");
        renderChat(0, "Welcome Bot", "You can type /join followed by anything to join a room.");
      }
    ...
    case "ROOM": {
      if(components[1]) {
        ...
        renderChat(0, "Welcome Bot", "Welcome back to the lobby!");
        room = null;
      }
      break;
    }
